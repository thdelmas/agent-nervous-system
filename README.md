...
The framing is the point: not a pile of tools, but a coherent set of self-regulating functions. The **consciousness-loop** is the executive at the center; the others are functions it schedules — discover (perception), remember (memory), protect (immunity), release (grief), and grow (development). Without the loop they're a body with no pulse; the loop is what makes them one agent. Two dualities tie the suite together: the loop is the **wake phase** to rem-sleep's **sleep phase** (the loop decides when to sleep), and **playtime** is the proactive complement to rem-sleep's reactive memory — sleep consolidates the experience you had, play generates new experience on purpose. Both feed memory; both grow the agent.
